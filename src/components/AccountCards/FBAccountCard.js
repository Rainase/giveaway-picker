...
const FBAccountCard = ({ fbAccounts, onClick }) => {
  return ( 
    <div className='flex flex-wrap mx-4'>
      { fbAccounts.length === 0 && <NoFacebookPages /> }
      {fbAccounts ? (
        fbAccounts.map((acc) => (
          <div
            key={acc.id}
            ...
            <div className='relative w-12 h-12 bg-white shadow-lg rounded-full'>
              ...
              <img className='rounded-full' src={acc.picture.data.url} alt={acc.name}/>
              </div>
            </div>
